"use strict";var _ethers=require("ethers");Object.defineProperty(exports,"__esModule",{value:!0}),exports.formatBN=exports.tenPow=void 0;function sameFraction(first,second){// Check that these two values are the same.
...
return first=first.replace(/0+$/,""),second=second.replace(/0+$/,""),_ethers.BigNumber.from(`1${first}`).eq(_ethers.BigNumber.from(`1${second}`))}const tenPow=x=>_ethers.BigNumber.from(10).pow(_ethers.BigNumber.from(x));exports.tenPow=tenPow;const formatBN=(amount,base,precision,fixed=!1)=>{// Inspired by: https://github.com/ethjs/ethjs-unit/blob/35d870eae1c32c652da88837a71e252a63a83ebb/src/index.js#L83
